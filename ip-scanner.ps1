...
Begin{
    #$IPv4Address = [IPAddress] (get-netadapter | get-netipaddress | ? addressfamily -eq 'IPv4').ipaddress
    $IPv4Address = [IPAddress] (Test-Connection -ComputerName $env:ComputerName -Count 1).IPV4Address.IPAddressToString
    $CIDR = [Int32] "24"
    $EnableMACResolving = [Switch] $True
    #$ExtendedInformations = [Switch] $True
    #$IncludeInactive = [Switch] $True
